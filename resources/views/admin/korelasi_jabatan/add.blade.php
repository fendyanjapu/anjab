...
<script>
	$(document).ready(function(){
		$('#jumlah_kolom').change(function(){
			...
			$.ajax({
				type   : "POST",
				url    : "/admin/korelasi-jabatan/jumlah_kolom",
				data   : {
                    '_token' : '{{ csrf_token() }}',
                    'jml' : jml
                },
				...
				success: function(response){
                    ...
                    for (var i = 0; i < jumlah; i++) {
                        html += '<div class="form-group row">' +
                            ...
                            '<div class="col-sm-9">' +
                                '<input type="text" name="nm_jabatan[]' + '" class="form-control" value="">' +
                            '</div>' +
                        '</div>';
                        html += '<div class="form-group row">' +
                                '<label class="col-sm-3 col-form-label">Unit Kerja/Instansi ' + '</label>' +
                                '<div class="col-sm-9">' +
                                    '<input type="text" name="unit_kerja_instansi[]' + '" class="form-control" value="">' +
                                '</div>' +
                            '</div>';
                        html += '<div class="form-group row border-bottom">' +
                                '<label class="col-sm-3 col-form-label">Dalam Hal ' + '</label>' +
                                '<div class="col-sm-9">' +
                                    '<input type="text" name="dalam_hal[]' + '" class="form-control" value="">' +
                                '</div>' +
                        '</div>';
                    }
...
style="position: relative;margin: auto;left:0;right:0;top:0; bottom:0;">
  <div class="card">
    <div class="card-body">
        ...
        action="{{ route('KorelasiJabatan.save') }}">
        @csrf
          ...
          <div id="kolom">
            <div class="form-group row">
              ...
              <label class="col-sm-3 col-form-label">Unit Kerja/Instansi</label>
              <div class="col-sm-9">
                <input type="text" name="unit_kerja_instansi" class="form-control">
              </div>
              <label class="col-sm-3 col-form-label">Dalam Hal</label>
              <div class="col-sm-9">
                <input type="text" name="dalam_hal" class="form-control">
              </div>
            </div>
          </div>
          <center>
