...
style="position: relative;margin: auto;left:0;right:0;top:0; bottom:0;">
  <div class="card">
    <div class="card-body">
        ...
        action="{{ route('jabatan_sopd.update', $data->id_jabatan) }}">
        @csrf
        @method('PUT')
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-9">
                <textarea  name="nama_jabatan" class="form-control" required>{{ $data->nama_jabatan }}</textarea>
              </div>
            </div>
            <div class="form-group row">
                ...
                <select class="form-control" name="id_unit_kerja" required>
                  <option value=""></option>
                        @foreach ($unit_kerja as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $data->id_unit_kerja ? 'selected' : '' }}>{{ $item->jenis_unit_kerja }}</option>
                        @endforeach
                </select>
              </div>
            </div>
